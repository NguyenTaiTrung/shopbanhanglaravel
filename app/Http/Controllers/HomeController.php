...
class HomeController extends Controller
{
    ...
    public function send_mail(){
        //send mail
                $to_name = "Nguyen_Tai_Trung";
                $to_email = "trung99001@gmail.com";//send to this email
        
                $data = array("name"=>"Mail từ tài khoản khách hàng","body"=>'Mail gửi về vấn đề hàng hóa'); //body of mail.blade.php
            
                Mail::send('pages.send_mail',$data,function($message) use ($to_name,$to_email){
                    $message->to($to_email)->subject('test mail google nhé');//send this mail with subject
     ...
     public function index(Request $request){
        ...
        //seo 
        $meta_desc = "Chuyên bán những đồ chơi điện tử chất lượng và phụ kiện chính hãng dành cho khách hàng"; 
        $meta_keywords = "do choi dien tu, phụ kiện điện tử";
        $meta_title = "Các loại đồ chơi điện tử và phụ kiện";
        ...
        $all_product = DB::table('tbl_product')->where('product_status','0')->orderby(DB::raw('RAND()'))->paginate(6); 
         $cate_pro_tabs = CategoryProductModel::where('category_parent','0')->orderBy('category_id','ASC')->get();
